  ...
  return (
    <div style={{ "flex-shrink": 0, padding: "0.25rem 1rem" }}>
      <div style={{ display: "flex", "align-items": "center", gap: "0.4rem" }}>
        ...
        <Show when={ab()}>
          {(v) => (
            <span style={{ "font-size": "0.8rem", color: "var(--fg)", "font-family": "monospace" }}>
              ↑{v().ahead} ↓{v().behind}
            </span>
          )}
        </Show>
        <Show when={ab() === null}>
          <span style={{ "font-size": "0.75rem", color: "var(--fg-muted)" }}>no upstream</span>
        </Show>

        <Show when={busy()}>
          <span style={{ "font-size": "0.8rem", color: "var(--accent)" }}>{busy()}…</span>
        </Show>
      </div>

      <Show when={busy() && progress()}>
        <p style={{ margin: "0.15rem 0 0", "font-size": "0.72rem", color: "var(--fg-muted)", "font-family": "monospace" }}>
          {progress()}
        </p>
      </Show>
      <Show when={err()}>
        <p style={{ margin: "0.15rem 0 0", color: "var(--error)", "font-size": "0.78rem", "white-space": "pre-wrap" }}>
          {err()}
        </p>
      </Show>
    </div>
